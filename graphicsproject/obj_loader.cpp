		...
		if (testIndex->vertexIndex == currentIndex->vertexIndex)
		{
			...
			for (unsigned int i = countStart; i < indexLookup.size() - countStart; i++)
			{
					...
					&& (!hasNormals || possibleIndex->normalIndex == currentIndex->normalIndex))
				{
					...
					{
						if ((currentPosition.x == result.positions[j].x && currentPosition.y == result.positions[j].y && currentPosition.z == result.positions[j].z)
							&& ((!hasUVs || (currentTexCoord.x == result.texCoords[j].x && currentTexCoord.y == result.texCoords[j].y)
							&& (!hasNormals || (currentNormal.x == result.normals[j].x && currentNormal.y == result.normals[j].y && currentNormal.z == result.normals[j].z)))))
						{
							return j;
						}
					}
